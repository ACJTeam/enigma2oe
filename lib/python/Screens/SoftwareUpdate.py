...
class UpdatePlugin(Screen, ProtectedScreen):
	...
	def checkTraficLight(self):
		...
		picon = None
		default = True
		url = "http://altoconsejojedi.es/status/%s-%s/" % (getImageVersion(), getImageType())
		...
		try:
			...
			message = _("The status of the current software could not be checked because %s can not be reached.") % ("http://altoconsejojedi.es")
			picon = MessageBox.TYPE_ERROR
			default = False
	...
	def showDisclaimer(self, justShow=False):
		if config.usage.show_update_disclaimer.value or justShow:
			message = _("En estos momentos, no esta estipulada la actualizacion del software de la imagen de forma automatica, en un futuro tal vez se implemente, disculpaen las molestias. Team ACJ")
	...
	def getLatestImageTimestamp(self):
		currentTimeoutDefault = socket.getdefaulttimeout()
		socket.setdefaulttimeout(3)
		url = "http://altoconsejojedi.es/status/%s-%s/buildtimestamp-%s" % (getImageVersion(), getImageType(), getBoxType())
			...
			if self.updating:
				...
				if self.total_packages:
					...
					if latestImageTimestamp:
						message = _("Latest available openacj %s build is from: %s") % (getImageVersion(), self.getLatestImageTimestamp()) + "\n"
